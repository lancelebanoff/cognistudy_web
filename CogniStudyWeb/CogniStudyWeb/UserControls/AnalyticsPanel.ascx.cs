...
{
    public partial class AnalyticsPanel : System.Web.UI.UserControl
    {
        public CogniPage mPage;
        UpdatePanel[] updatePanels;
        UpdatePanel[] catUpdatePanels;

        public bool ShowStudentDropdown { get { return Convert.ToBoolean(ViewState["ShowStudentDropdown"]); } set { ViewState["ShowStudentDropdown"] = value; } }
        public string SelectedStudent { get { return Page.Request.QueryString["StudentId"]; } }
        ...
        private async Task FillFilterDropdowns()
        {
            ...
            if (ShowStudentDropdown)
            {
                ...
                pnlStudentDropdown.Visible = true;
            }
            else
            {
                //PublicUserData studentPublicData = await PublicUserData.GetById(SelectedStudent);
                //ListItem item = new ListItem(studentPublicData.DisplayName, studentPublicData.ObjectId);
                //ddlFilterStudent.Items.Insert(0, item);
                ddlFilterStudent.Items.Insert(0, SelectedStudent);
                pnlStudentDropdown.Visible = false;
            }
        }
        ...
        private async Task<object> GetStudentDataSource()
        {
            ...
            IList<PublicUserData> students = mPage.PrivateTutorData.Students;
        ...
        protected void btnUpdatePanels_Click(object sender, EventArgs e)
        {
            ...
            AsyncHelpers.RunSync(BuildCharts);
        }
